...
void ScanManager::Scan(const string& path)
{
	...
	{
		if (*localit < *dbit)//本地有，数据库没有，新增本地数据到数据库
		{
			_datamgr.InsertDoc(path, *localit);//对应到中的数据库
			++localit;
		}
		else if (*localit>*dbit)//本地没有，数据库有，删除数据库数据
		{
			_datamgr.DeleteDoc(path, *dbit);//对应到中的数据库
			++dbit;
		}
		else
		{
	...
	{
		_datamgr.DeleteDoc(path, *dbit);//对应到中的数据库中删除应该注意整个文件夹都删了怎么办
		++dbit;
	}

	//递归比对子目录数据
	for (const auto& subdirs : localdirs) //localdirs 存目录的vector
	{
		string subpath = path;
		subpath += '\\';
		subpath += subdirs; 

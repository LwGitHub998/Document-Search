...
#include <io.h>
#include <set>
#include <Windows.h>//后面加高亮了会用到的头文件
#include <stdio.h>
#include <thread>
#include <mutex>
using std::cout;
...
//防止多个文件都有这个函数
static void DirectoryList(const string& path, vector<string>& dirs, vector<string>& files)//static只在当前文件可见
{
	_finddata_t file;
	const string path_ = path + "\\*.*";//转义字符，因为这里要找某个目录下的文件，所以要找到第一个子文件
	intptr_t handle = _findfirst(path_.c_str(), &file);//找到目录下的第一个孩子
	//返回值：如果查找成功的话，将返回一个long型的唯一的查找用的句柄（就是一个唯一编号）。这个句柄将在_findnext函数中被使用。若失败，则返回-1。
	if (handle == -1)//_findfirst成功返回0，失败返回-1
	{
	...
	do{
		...
		if (file.attrib & _A_SUBDIR)//判断是一个文件夹  是则加入目录vector
		{
			if ((strcmp(file.name, ".") != 0) && (strcmp(file.name, "..") != 0))
				//因为每个文件夹内默认包含了当前和上一层，所以不再遍历当前和上层，否则会有死循环
			{
	...
	} while (_findnext(handle, &file) == 0);//找同一层的文件
	//_findclose(handle);
}
//
...
#define ERROE_LOG(...) \
	__ErrorDebug(__FILE__, __LINE__, __FUNCTION__, __VA_ARGS__);//可变参数


//控制高亮部分
//只改变这一点的颜色，输出完成后再恢复回去
static void ColourPrintf(const char* str)
{
	// 0-黑1-蓝2-绿3-浅绿4-红5-紫6-黄7-白8-灰9-淡蓝10-淡绿
	// 11-淡浅绿12-淡红13-淡紫14-淡黄15-亮白
	//颜色：前景色+ 背景色*0x10
	//例如：字是红色，背景色是白色，即红色+ 亮白= 4 + 15*0x10
